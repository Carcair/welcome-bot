  ...
  queries.selectAll()
    .then(result => {
      ...
      temp.forEach((obj, index) => {
        const schedule = new Schedules(obj.message, obj.run_date, obj.repeat_range);
        // Decode query return
...
router.get('/:title', (req, res) => {
  queries.selectOne('message', req.params.title)
    .then(result => {
      // Decode query return
      let temp = JSON.parse(result);
      let schedule = new Schedules(temp[0].message, temp[0].run_date, temp[0].repeat_range);
 ...
 */
router.post('/', (req, res) => {
  const schedule = new Schedules(req.body.message, req.body.run_date, req.body.repeat_range);
